...
def find_approx_single_vent(subjectlist3_unableevents, row): 
    
    """
    subjectlist3_unableevent 테이블에서 현재 행의 결측된 intubationtime 또는 extubationtime의 대체 가능한 값을 찾습니다. 
    
    Parameters:
    - subjectlist3_unableevents: subjectlist3_unableevents 테이블
    ...
    if pd.isnull(row['extubationtime']):
        target_time = 'extubationtime(un)'
    else: #intubationtime이 없으면 
        target_time = 'intubationtime(un)'
        
    if target_time == 'extubationtime(un)':
        known_time = row['intubationtime']
    else: # target_time이 intubationtime(un)이면 
        known_time = row['admittime'] 
        
    if target_time == 'extubationtime(un)':
        compare_time = row['dischtime'] # 퇴원시간과 비교해 
    else:
        compare_time = row['extubationtime'] # 발관 시간과 비교해 
    
    # 원래 시간 기록
    results['original_times'] = {'intubationtime': row.get('intubationtime'), 'extubationtime': row.get('extubationtime')} #이중 딕셔너리
    
    # 결측 시간에 따라 후보 필터링
    candidates = subjectlist3_unableevents[subjectlist3_unableevents['stay_id'] == stayid]
    candidates = candidates[(candidates[target_time] > known_time) & (candidates[target_time] < compare_time)] 
    
    if len(candidates) > 1:
        candidates['time_diff'] = (candidates[target_time] - known_time).abs() # 절댓값은 왜 하는거지 ??
        ...
        corrected_time_type = "extubationtime" if target_time == "extubationtime(un)" else "intubationtime"
        
        if len(closest_candidates) == 1: # 당연히 하나 아닌가요..?
            # 가장 적합한 후보 시간 기록
...
def find_best_candidate_for_imputation(subjectlist3_unableevents, row, prev_row=None, next_row=None):
    """
    ...
    if pd.isnull(row['intubationtime']):
        candidates = subjectlist3_unableevents[(subjectlist3_unableevents['stay_id'] == stay_id) &
                                    (subjectlist3_unableevents['intubationtime(un)'] < row.get('extubationtime', np.inf)) &
                                    (subjectlist3_unableevents['intubationtime(un)'] > prev_row.get('extubationtime', row['admittime']))]
        
        if not candidates.empty:
            # 여러 intubationtime(un)이 존재할 경우 그들의 'extubationtime'을 참조해서 비교
            candidates['time_diff'] = (candidates['extubationtime(un)'] - row.get('extubationtime', np.inf)).abs()
            best_candidate_idx = candidates['time_diff'].idxmin()
            results.append(('intubationtime', candidates.loc[best_candidate_idx, 'intubationtime(un)'], candidates.loc[best_candidate_idx, 'stay_id']))
            
    elif pd.isnull(row['extubationtime']):
        candidates = subjectlist3_unableevents[(subjectlist3_unableevents['stay_id'] == stay_id) &
                                    (subjectlist3_unableevents['extubationtime(un)'] > row.get('intubationtime', -np.inf)) &
                                    (subjectlist3_unableevents['extubationtime(un)'] < next_row.get('intubationtime', row['dischtime']))]
        if not candidates.empty:
            # 여러 extubationtime(un)이 존재할 경우 그들의 'starttime'을 참조
            candidates['time_diff'] = (candidates['intubationtime(un)'] - row.get('intubationtime', -np.inf)).abs()
            best_candidate_idx = candidates['time_diff'].idxmin()
            results.append(('extubationtime', candidates.loc[best_candidate_idx, 'extubationtime(un)'], candidates.loc[best_candidate_idx, 'stay_id']))

    return results


def process_ventilation_sequences(subjectlist3_unableevents, group):
    """
    한 환자(=group)의 subjectlist3_unableevents 이벤트를 처리하여 누락된 데이터에 대한 최적의 후보를 찾습니다.
    ...
    for idx, row in group.iterrows():
        ...
        if row['seq_num'] > 1:
            # 현재 행(row)의 seq_num보다 1 작은 값의 행을 찾아 prev_row에 할당
            prev_row = group[group['seq_num'] == row['seq_num'] - 1].iloc[0]
        else:
            # seq_num이 1 이하일 경우 빈 Series를 prev_row에 할당
            prev_row = pd.Series()
            
        if row['seq_num'] < group['seq_num'].max():
        # 현재 행(row)의 seq_num보다 1 큰 값의 행을 찾아 next_row에 할당
            next_row = group[group['seq_num'] == row['seq_num'] + 1].iloc[0]
        else:
        # seq_num이 최대값일 경우 빈 Series를 next_row에 할당
            next_row = pd.Series()

        current_pair = {'intubationtime': row.get('intubationtime'), 'extubationtime': row.get('extubationtime')}

        if row.get('reint_marker', False): # 이게 지금 reint_marker가 없는거 같은데 ..? 
            candidates = find_best_candidate_for_imputation(subjectlist3_unableevents, row, prev_row, next_row)
            if candidates:
...
def subjectlist3_search(subjectlist3_unableevents, subject_df):
    """
    subjectlist3_unableevents 테이블을 이용해 결측된 삽관/발관 시간을 찾는 메인 함수.
    """
    ...
    for (subject_id, hadm_id), group in grouped_df:
        if len(group) == 1:
            ...
            single_row_results = find_approx_single_vent(subjectlist3_unableevents, single_row)
            ...
            multirow_candidates = process_ventilation_sequences(subjectlist3_unableevents, group)
...
def find_missing_intubationtime(df, subjectlist3_unableevents):
    ...
    for hadm_id, group in df.groupby('hadm_id'):
        for idx, row in group.iterrows():
            if pd.isnull(row['intubationtime']):
                stay_id = row['stay_id']
                subjectlist3_events = subjectlist3_unableevents[subjectlist3_unableevents['stay_id'] == stay_id]

                if idx == group.index[0]:  # First row in the group
                    valid_intubationtimes(UnicodeError) == subjectlist3_events[
                        (subjectlist3_events['intubationtime(un)'] >= row['admittime']) & 
                        (subjectlist3_events['intubationtime(un)'] <= row['extubationtime'])
                    ]
                else:  # Not the first row, compare with previous row's extubationtime
                    prev_row = df.loc[idx - 1]
                    valid_intubationtimes = subjectlist3_events[
                        (subjectlist3_events['intubationtime(un)'] >= prev_row['extubationtime']) & 
                        (subjectlist3_events['intubationtime(un)'] <= row['extubationtime'])
                    ]

                # If valid starttime values are found, save them along with the index
                for intubationtime in valid_intubationtimes['intubationtime(un)'].values:
                    missing_intubation_times.append({'index': idx, 'intubationtime(un)': intubationtime})
...
def impute_missing_intubationtime(df, missing_intubation_times):
    for item in missing_intubation_times:
        idx = item['index']
        intubationtime= item['intubationtime(un)']
        
        # Ensure the starttime is converted to a Timestamp if not already
        if not isinstance(intubationtime, pd.Timestamp):
            intubationtime = pd.to_datetime(intubationtime)
        
        # Impute the missing intubationtime
        df.at[idx, 'intubationtime'] = intubationtime
    
    return df


def impute_candidates(df, single_row_results_list, multirow_candidates_list, subjectlist3_unableevents):
    ...
    for entry in single_row_results_list + multirow_candidates_list:
        for candidate_info in entry['candidates']:
            ...
            for i, (time_type, time_value) in enumerate(candidate_info['candidates']):
                ...
                log_message = f"{time_type} unable imputation"
    ...
    missing_intubation_times = find_missing_intubationtime(df, subjectlist3_unableevents)
